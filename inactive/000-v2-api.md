...
The problem with serializing JSON from the views is:

- Expensive to locate the views in the file structure.
- JSON serialization is done through a response/request life cycle.
...
What if the users project is not using Ember?
...
The [JSON API Spec](http://jsonapi.org) details best practices when constructing an API.
For the second version of Sprees' API, the JSON API Spec will allow developers to expect a consistent and standardized JSON API.
